...
class AppLoaderTest < ActiveSupport::TestCase
  ...
  def expects_exec(exe)
    assert_equal [Rails::AppLoader::RUBY, exe, *ARGV], loader.exec_arguments
  end
  ...
  ["bin", "script"].each do |script_dir|
    ...
    if exe == "bin/rails"
      test "warns and boots rails commands for Bundler-generated #{exe}" do
        write exe, "This file was generated by Bundler"
        write "config/boot.rb", "$app_loader_boot_required = true\n"

        commands = File.expand_path("rails/commands.rb", @tmp)
        write commands, "$app_loader_commands_required = true\n"
        $LOAD_PATH.unshift(@tmp)

        err = capture(:stderr) { loader.exec_app }

        assert_match(/Rails ships with a `rails` binstub/, err)
        assert_equal File.expand_path("config/application", @tmp), APP_PATH
        assert $app_loader_boot_required
        assert $app_loader_commands_required
      ensure
        Object.send(:remove_const, :APP_PATH) if Object.const_defined?(:APP_PATH)
        $LOAD_PATH.delete(@tmp)
        $app_loader_boot_required = nil
        $app_loader_commands_required = nil
      end
    end
  end
