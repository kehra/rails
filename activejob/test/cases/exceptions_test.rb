...
class ExceptionsTest < ActiveSupport::TestCase
  ...
  class RaisingAfterDiscardJob < ActiveJob::Base
    ...
    after_discard { raise FirstAfterDiscardHookError, "first" }
    after_discard { raise SecondAfterDiscardHookError, "second" }

    def perform
      raise AfterDiscardHookDiscardableError
    end
  end
  ...
  unless adapter_skips_scheduling?(ActiveJob::Base.queue_adapter)
    ...
    test "after_discard reraises the last error from discard hooks" do
      raised = assert_raises(SecondAfterDiscardHookError) do
        RaisingAfterDiscardJob.perform_later
      end

      assert_equal "second", raised.message
    end
