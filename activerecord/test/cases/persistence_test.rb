...
class PersistenceTest < ActiveRecord::TestCase
  ...
  def test_update_one_with_id
    topic = Topic.update(1, "content" => "1 updated")

    assert_equal 1, topic.id
    assert_equal "1 updated", topic.content
    assert_equal "1 updated", Topic.find(1).content
  end
  ...
  def test_create_bang_through_factory_with_block
    topic = Topic.create!("title" => "New Topic") do |t|
      t.author_name = "David"
    end
  ...
  def test_create_bang_many_through_factory_with_block
    topics = Topic.create!([{ "title" => "first" }, { "title" => "second" }]) do |t|
      t.author_name = "David"
    end
