...
class DateExtCalculationsTest < ActiveSupport::TestCase
  ...
  def test_to_fs_with_custom_date_format
    ActiveSupport::DateFormats.register(:custom_ymd, "%Y%m%d")

    assert_equal "20050221", Date.new(2005, 2, 21).to_fs(:custom_ymd)
  end
  ...
  def test_date_formats_register_adds_custom_date_format
    ActiveSupport::DateFormats.register(:registered_custom_date_format, "%B %Y")

    assert_equal "February 2005", Date.new(2005, 2, 21).to_fs(:registered_custom_date_format)
  end
